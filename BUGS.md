...
## Status overview (updated 2026-04-14 after Phase 17 shipped)

| # | Title | Status | Closed by |
|---|---|---|---|
| **#1** | `derive_variables` audit attribution misleads on agent provenance | ✅ **CLOSED** | Phase 17.2 — commit `c9f9f3d` (per-variable audit emission, 3 new AuditAction values) |
| **#2** | `/ground_truth` endpoint conflates two distinct 404 states | ✅ **CLOSED** | Phase 17.3 — commit `a560845` (dual 404 via new `PipelineInterpreter.completed_steps` property) |
